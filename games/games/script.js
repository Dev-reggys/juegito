...
let currentUser = ""; // Se llenará al hacer login
...
/* --- 8. AUTH Y LOGIN (CON NOMBRE EN BIENVENIDA) --- */
function ejecutarAuth() {
    ...
    if (!user || !pass) {
        alert("¡Cuidado! Ingresa un usuario y una contraseña.");
        return;
    }

    if (modoRegistro) {
        // --- LÓGICA DE REGISTRO COMPLETADA ---
        if (usuarios[user]) {
            alert("Ese jugador ya existe. ¡Elige otro nombre!");
        } else {
            usuarios[user] = pass; // Asignamos la contraseña al usuario
            localStorage.setItem('usuarios_arcade', JSON.stringify(usuarios)); // Guardamos en el navegador
            alert("¡Registro exitoso! Ya puedes iniciar sesión.");
            alternarModoAuth(); // Devolvemos la pantalla a modo Login automáticamente
        }
    } else {
        ...
        if (usuarios[user] === pass) {
            ...
            alert("Credenciales Incorrectas. ¿Olvidaste tu contraseña?");
        }
    }
}
...
function alternarModoAuth() {
    ...
    // Cambiamos el texto del botón principal de acción
    // NOTA: Asegúrate de que el ID de tu botón principal en el HTML sea 'auth-main-btn'
    const btnPrincipal = document.getElementById('auth-main-btn'); 
    if (btnPrincipal) {
        btnPrincipal.innerText = modoRegistro ? "CREAR CUENTA" : "ENTRAR";
    }

    // Cambiamos el texto del texto que te deja cambiar de modo
    // NOTA: Asegúrate de que el ID en tu HTML sea 'switch-btn'
    const switchBtn = document.getElementById('switch-btn');
    if (switchBtn) {
        switchBtn.innerText = modoRegistro ? "Inicia sesión aquí" : "Regístrate aquí";
    }
}
...
/* --- 9. MOTORES DE JUEGO (Resto de funciones igual) --- */
/* [AQUÍ SIGUEN TUS FUNCIONES initSnake, initDino, initGuitarHero, etc., sin cambios] */

// (El resto de tus funciones initSnake, initDino, initGuitarHero, etc. se mantienen idénticas a tu código anterior)
...
function activarJumpscare() {
    ...
    if (jumpVideo) {
        ...
        jumpVideo.style.cssText = `display: block !important; position: fixed !important; top: 0; left: 0; width: 100vw; height: 100vh; background: black; z-index: 999999; object-fit: cover;`;
        jumpVideo.muted = false; jumpVideo.volume = 1.0; jumpVideo.currentTime = 0;
        jumpVideo.play().catch(e => console.error(e));
        jumpVideo.onended = () => {
            jumpVideo.style.display = 'none'; jumpVideo.muted = true; 
            if(!window.gameActive && window.audioTracks['menutheme']) window.audioTracks['menutheme'].play();
        };
    }
}

document.addEventListener('click', (e) => {
    if (e.target.classList.contains('nyan-cat') || e.target.classList.contains('welcome-gif')) activarJumpscare();
});

document.addEventListener('DOMContentLoaded', () => {
    const v = document.querySelector('video.nyan-cat');
    if (v) { v.style.display = 'none'; v.loop = false; }
});